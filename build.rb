...
unless(ARGV.include? "--legacy")
    ...
    if ENV["YI_TSNODE_SCRIPTS"]
        ...
        status = system("./tools/youi-cli/node_modules/.bin/ts-node", "tools/youi-cli/commands/cmdbuild/index.ts", *ARGV)
        exit status ? 0 : 1
    end

    status = system("node", "tools/youi-cli/dist_commands/cmdbuild/index.js", *ARGV)
    result = $?
    if status
        exit 0
    end
