...
unless(ARGV.include? "--legacy")
    ...
    if((argStr.match(/-p android/) || argStr.match(/--platform android/)) && !argStr.match(/--working_directory/))
        ARGV.append("--working_directory")
        ARGV.append(appDir)
    end

    if ENV["YI_TSNODE_SCRIPTS"]
        puts "Running ts-node scripts!"
        status = system("./tools/youi-cli/node_modules/.bin/ts-node", "tools/youi-cli/commands/generate/index.ts", *ARGV)
        exit status ? 0 : 1
    end
    
    status = system("node", "tools/youi-cli/dist_commands/generate/index.js", *ARGV)
    result = $?
    if status
        exit 0
    end
    # Check to ensure that node exists, otherwise you end up getting no printed error.
    has_node = system("node --version", [:out, :err] => File::NULL)
    if has_node
        exit 1
    end
    puts "node missing. Please install the required version of node."
    exit 1
end

ARGV.delete_at(ARGV.index("--legacy"))
##
...
class GenerateOptions
    def self.parse(args)
        ...
        platformList = ["android", "ios", "linux", "osx", "ps4", "tizen-nacl", "tvos", "uwp", "webos3", "webos4", "webos5", "webos6", "win64"]
        ...
        begin
            ...
            if options.generator.nil?
                ...
                when /ps4/i
                    options.generator = "Visual Studio 16 2019\" -A\"ORBIS"
                when /Tizen-NaCl/i
    ...
    def self.create_cmake_command(options)
        ...
        case options.platform
        when /ps4|uwp|win64/i
            # PS4, UWP, WIN64 use the built-in version of CMake, so we need to reference that
